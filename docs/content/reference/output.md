...
describes what users get. Pick a format with `-o`, or let npmjs choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
npmjs <command> -o table   # aligned columns for reading
npmjs <command> -o jsonl   # one JSON object per line, for piping
npmjs <command> -o json    # a single JSON array
npmjs <command> -o csv     # spreadsheet friendly
npmjs <command> -o tsv     # tab-separated
npmjs <command> -o url     # just the URL column
npmjs <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
npmjs <command> --fields id,title,url
```
...
```bash
npmjs <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
npmjs <command>            # a table, because this is a terminal
npmjs <command> | wc -l    # JSONL, because this is a pipe
```
